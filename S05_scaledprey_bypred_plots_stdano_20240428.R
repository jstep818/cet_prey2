...
mult_fact <- data.frame("grp_nm" = c("sm_zoop", "lg_zoop", "shrimp", "euphausiid"),
                        "scalar" = c(0.1, 10, 100, 10))
ZPD_fall_sf <- left_join(ZPD_fall_sf, mult_fact, by=c("group_multi" = "grp_nm"))

ZPD_fall_sf$BIOMASS <- ZPD_fall_sf$ABUNDANCE * ZPD_fall_sf$scalar
...
zpd_clusteryear$YEAR <- zpd_clusteryear$YEAR
...
sss <- 5 #relational of axes of biomass and abundance
...
zpd_strat_mean %>% filter(group_multi!=c("")) %>%
  group_by(YEAR) %>%
  group_by(group_multi) %>%
  mutate(freq = (strat.biomass - mean(strat.biomass))/ strat.biomass) %>% 
  ggplot(aes(fill=factor(group_multi), y=freq, x=YEAR)) + 
  geom_bar(position="dodge", stat="identity") #+
  #facet_wrap(.~group_multi)
...
setwd("/Users/jstepanuk/Dropbox/my_papers/InPrep_Stepanuketal_GOMprey")
pdf("zoop_prey_mnba_stdano_20240414.pdf", width=7, height=6)
zpd_out %>%
  filter(group_multi != "") %>%
  filter(mnba_prey == "Preferred Prey") %>%
  ggplot(aes(fill=factor(group_multi, levels = c("euphausiid", "shrimp", "lg_zoop", "sm_zoop"), labels = c("Euphausiid", "Shrimp", "Large Zooplankton", "Small Zooplankton")), y=std_ano, x=YEAR)) + 
  geom_bar(position="dodge", stat="identity") +
  facet_wrap(.~factor(stratcat, levels=c("GOMstrat", "GBstrat", "SNEstrat", "MABstrat"), labels = c("Gulf of Maine", "George's Bank", "Southern New England", "Mid-Atlantic Bight"))) +
  labs(x="Year", y="Std. Anomaly", fill = "") +
  theme_minimal() +
  scale_fill_manual(values= c("#0A81D1", "#A3CCA6")) +
  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1),
        text=element_text(size=16),
        legend.position = "bottom") 
dev.off()
