...
pdf("bts_biomass_groups_scaledtotal_20221022_EVENAXES.pdf", width=8, height=6)
t_region %>% filter(!is.na(group)) %>%
  group_by(stratcat,YEAR) %>%
  mutate(freq = strat.biomass / sum(strat.biomass)) %>% 
  ggplot(aes(fill=factor(group2), y=strat.biomass, x=YEAR)) + 
  geom_bar(position="stack", stat="identity") +
  facet_wrap(.~factor(stratcat, levels=c("GOMstrat", "GBstrat", "SNEstrat", "MABstrat"), labels = c("Gulf of Maine", "George's Bank", "Southern New England", "Mid-Atlantic Bight"))) + #, scales="fixed"
  theme_minimal() +
  theme(axis.text.x = element_text(angle=45, hjust=1, vjust=1),
...
t_region2$dumyr <- ifelse(t_region2$YEAR == "1981-1985", 1, NA)
t_region2$dumyr <- ifelse(t_region2$YEAR == "1986-1990", 2, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "1991-1995", 3, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "1996-2000", 4, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "2001-2005", 5, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "2006-2010", 6, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "2011-2015", 7, t_region2$dumyr)
t_region2$dumyr <- ifelse(t_region2$YEAR == "2016-2019", 8, t_region2$dumyr)

group_mm_lm_outs <- NULL
for(i in unique(t_region2$variable)){
  ppz <- t_region2[t_region2$variable == i,]
  for(j in  unique(t_region2$stratcat)){
    pppz <- ppz[ppz$stratcat == j,]
    
    lm_Z <- lm(value ~ YEAR, data=pppz)
    p1 <- c("YEAR",i, j, summary(lm_Z)$coefficients[1], 
            summary(lm_Z)$coefficients[2], 
            summary(lm_Z)$coefficients[8], 
            summary(lm_Z)$coefficients[4],  
            summary(lm_Z)$coefficients[6], 
            summary(lm_Z)$adj.r.squared)
    
    group_mm_lm_outs <- rbind(group_mm_lm_outs, p1)
    
  }
}
group_mm_lm_outs <- data.frame(group_mm_lm_outs)
colnames(group_mm_lm_outs) <- c("LAB", "variable", "region", "int_est", "year_slope", "year_p", "year_std_err", "year_t_val", "adj_r2")
...
setwd("/Users/jstepanuk/Dropbox/my_papers/InPrep_Stepanuketal_GOMprey")
pdf("zoop_biomass_groups_scaledtotal_20220723.pdf", width=8, height=6)
zpd_region %>% filter(group_multi!=c("")) %>%
  group_by(stratcat, YEAR) %>%
  mutate(freq = strat.biomass / sum(strat.biomass)) %>% 
  ggplot(aes(fill=factor(group_multi2), y=freq, x=YEAR)) + 
